...
import {
  LayoutDashboard, FolderOpen, Users, BarChart3,
  FileText, Bell, LogOut, ClipboardList, X,
} from "lucide-react"
...
const candidatLinks = [
  { to: "/candidat/dashboard",   label: "Tableau de bord", icon: LayoutDashboard },
  { to: "/candidat/inscription", label: "Mon inscription",  icon: ClipboardList },
...
export default function Sidebar() {
  ...
  const links =
    ...
    role === Role.AGENT ? agentLinks :
    candidatLinks

  const roleLabel =
    role === Role.ADMIN ? "Administration" :
    role === Role.AGENT ? "Espace Agent" :
    "Espace Candidat"
    ...
    <aside className="fixed left-0 top-0 h-screen w-44 bg-sidebar flex flex-col z-50">
      ...
      <div className="px-4 py-5 border-b border-neutral-700">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-white text-sm font-bold leading-tight">Inscription</p>
      ...
      <nav className="flex-1 px-3 py-4 space-y-1">
        <p className="text-neutral-500 text-xs font-semibold uppercase px-2 mb-2">Principal</p>
      ...
      <div className="px-3 py-4 border-t border-neutral-700">
        <div className="flex items-center gap-2 px-2 mb-3">
          ...
          <div className="relative">
            <button onClick={handleBell} className="relative p-1 hover:bg-neutral-800 rounded transition-colors">
            ...
            {showNotifs && (
              <div className="fixed bottom-16 left-44 w-72 bg-white rounded-xl shadow-xl border border-neutral-200 z-50 overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-100">
                  ...
                  <div className="flex items-center gap-2">
                    {notifications.length > 0 && (
                      <button onClick={clear} className="text-xs text-neutral-400 hover:text-neutral-600">
                        Tout effacer
                      </button>
                    )}
                    <button onClick={() => setShowNotifs(false)} className="text-neutral-400 hover:text-neutral-600">
                      <X size={14} />
                    </button>
                  </div>
                </div>
                <div className="max-h-64 overflow-y-auto">
                  {notifications.length === 0 ? (
                    <p className="text-neutral-400 text-sm text-center py-6">Aucune notification</p>
                  ) : (
                    notifications.map((n, i) => (
                      <div key={i} className="px-4 py-3 border-b border-neutral-50 hover:bg-neutral-50 transition-colors">
                        <p className="text-sm text-neutral-900">{n.message}</p>
                        <p className="text-xs text-neutral-400 mt-1">
                          {new Date(n.date ?? n.timestamp ?? "").toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })}
                        </p>
                      </div>
                    ))
                  )}
